...
- `cmd/bds`: entrypoint.
- `internal/bootstrap`: flags, config, logging and process shutdown wiring.
...
- `internal/server`: core server lifecycle, tick loop, main-thread task queue, MCPE login parsing, encryption handshake, resource-pack/spawn/world/player-sync/chat/command session logic and packet routing.
...
1. Bootstrap loads config and builds `internal/server.Server`.
2. `internal/server.Server` starts the core tick loop, owns the MCPE handler and starts `internal/network/mcpe` on the configured RakNet address with a server-provided client factory.
3. `internal/network/mcpe` configures MOTD/pong data and accepts sessions through `internal/network/raknet`; online player count updates are pushed from the server lifecycle callbacks.
...
8. `internal/server` routes chat and command packets through the local command registry; the core server routes console input to the same registry with a console sender.
9. `internal/server.Server` controls shutdown, listener lifetime, tick metrics, main-thread tasks and basic crash diagnostics.
